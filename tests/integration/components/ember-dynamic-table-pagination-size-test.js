...
test('it renders', function(assert) {
  ...
  assert.equal(this.$('.pageSize select').length, 1, 'it exists');
});
...
test('The select list renders an option for each of the values in the pageSizeOption array', function(assert) {
  ...
  assert.equal(selectOptions.length, 4, 'correct number of select options rendered');
...
test("The span text displays 'Rows per page:'", function(assert) {
  assert.expect(1);
  this.render(hbs`{{ember-dynamic-table-pagination-size}}`);
  assert.equal(this.$('.pageSize span').text().trim(), 'Rows per page:', 'The correct span text is displayed');
});
...
test('When a new pageSize is selected the new size is sent up through the setPageSize action', function(assert) {
  ...
  this.render(hbs`{{ember-dynamic-table-pagination-size pageSize=pageSize pageSizeOptions=pageSizeOptions 
              onPageSizeChange=(action 'mockSetPageSize')}}`);
